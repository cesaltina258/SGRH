...
const prop = defineProps({
  ...
  data: {  // Adicione esta propriedade
    type: Object,
    default: null
  }
});
  ...
  <v-dialog :width="width" v-model="dialogValue">
    <v-card>
      ...
      <v-card-text class="text-center ma-md-5">
        <div class="text-info">
          <i class="ph ph-copy ph-4x" />
        </div>
        <div class="mt-4">
          <h4 class="text-h6 font-weight-bold">
            <slot name="title"> {{ $t('t-dialog-title-confirm-clone') }}</slot>
          </h4>
          <p class="text-muted mx-4 mb-0 text-subtitle-1">
            <slot name="text">
              {{ $t('t-dialog-text-confirm-clone') }}
            </slot>
          </p>
        </div>
      </v-card-text>
