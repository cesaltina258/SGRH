<template>
    <div class="data-table-server-wrapper">
      ...
      :headers="processedHeadersWithSelect"
        ...
        <template #bottom>
          <div v-if="showPagination" class="d-flex justify-space-between align-center mt-4">
            <div class="text-muted">
              {{ $t('t-showing') }} <b>{{ (page - 1) * itemsPerPage + 1 }}-{{ Math.min(page * itemsPerPage, totalItems) }}</b> {{ $t('t-of') }}
  ...
  const processedHeadersWithSelect = computed(() => {
  // Adiciona a coluna de seleção automaticamente
  return [
    { 
  ...
  const loadItems = async ({ page: newPage, itemsPerPage: newItemsPerPage, sortBy }) => {
    page.value = newPage
