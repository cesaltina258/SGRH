...
import CreateUpdateAgentDialog from "@/components/baseTables/sector/CreateUpdateAgentDialog.vue";
...
import editSector from "@/components/baseTables/sector/editSector/index.vue";
...
const onEdit = (item: AgentListingType) => {
  router.push({
    path: "/baseTable/edit-sector",
    query: {
      name: item.name,
    },
  });
      ...
      <v-row justify="space-between">
        ...
        <v-col lg="auto">
          <v-btn color="secondary" @click="onCreateEditClick(null)">
            <i class="ph-user-plus me-1" /> {{ $t('t-add-sector') }}
          </v-btn>
        </v-col>
      </v-row>
    </v-card-title>
