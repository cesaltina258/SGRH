    ...
    <div class="w-100">
      <v-card-title class="text-center mt-10">
        ...
        <div class="text-muted mt-1 font-weight-regular">
          {{$t('t-enter-the-4-digit-code')}} <b>example@abc.com</b>
        </div>
      </v-card-title>
      <v-card-text>
              ...
              <v-col cols="3" v-for="(item, index) in form" :key="index">
                <div class="mb-3">
                  <TextField class="two-step-verification-field" v-model="item.value" :id="`digit-${index + 1}`"
                    placeholder="0" maxLength="1" isRequired :showError="true" :isSubmitted="isSubmitted" hide-details
                    @keyup="moveToNext($event, index + 2)" @keyup.delete="moveToPrev($event, index)" />
                </div>
              </v-col>
            </v-row>
          </v-col>
        ...
        <v-row justify="center" class="align-center">
          <v-col cols="12" lg="8">
            <v-btn color="primary" block height="40" @click="isSubmitted = true">
              {{ $t('t-confirm') }}
            </v-btn>
          </v-col>
        </v-row>
